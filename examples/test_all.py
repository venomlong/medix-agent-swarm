...
async def test_deep_research_tool_integration():
    ...
    # 检查工具注册（skill_registry.get_all() 返回 {技能名: 元数据} 字典）
...
async def test_harness_entropy_manager():
    ...
    old_memories = [
        {"memory_id": "m1", "content": "旧记忆1", "timestamp": now - timedelta(days=200)},  # 过期
        {"memory_id": "m2", "content": "旧记忆2", "timestamp": now - timedelta(days=100)},  # 过期
        {"memory_id": "m3", "content": "新记忆", "timestamp": now - timedelta(days=5)},   # 有效
    ]
